...
def stable_kodi(abi, directory):
    ...
    matches = [
        (
            (int(item[2]), int(item[3]), int(item[4] or 0)),
            item[1],
            item[0],
        )
        for item in re.findall(pattern, page, re.I)
        if not re.search(r"alpha|beta|rc", item[0], re.I)
    ]
    if not matches:
        raise RuntimeError(f"No stable Kodi build found at {directory}")
    _, version, filename = max(matches)
    return {"version": version, "url": directory + filename, "sha256": None, "signerSha256": None}

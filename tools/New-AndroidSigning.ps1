...
$alias = 'starlane-movies'
$keytool = Join-Path $env:JAVA_HOME 'bin\keytool.exe'
if (-not (Test-Path $keytool)) { throw 'JAVA_HOME must point to JDK 17 before generating the Android signing key.' }

& $keytool -genkeypair -noprompt -keystore $keystore -storetype PKCS12 -storepass $password -keypass $password -alias $alias -keyalg RSA -keysize 4096 -validity 10000 -dname 'CN=Starlane Movies, O=Starlane Movies, C=GB'

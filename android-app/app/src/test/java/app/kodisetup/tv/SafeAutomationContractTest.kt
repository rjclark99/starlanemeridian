...
class SafeAutomationContractTest {
    @Test fun `strict flow preserves visible confirmations and profile boundary`() {
        ...
        assertTrue(activity.contains("Continue to confirmation"))
        assertTrue(activity.contains("Confirm and approve one run"))
        ...
        assertTrue(viewModel.contains("pending_install_package"))
        assertTrue(viewModel.contains("resumePendingInstall()"))
        assertTrue(viewModel.contains("bootstrapPreparation?.isActive == true"))
        assertFalse(viewModel.contains("BootstrapRecoveryPolicy.shouldRetry"))
        assertTrue(activity.contains("Resume automated setup"))
        assertTrue(activity.contains("if (!state.automationRunning)"))
        assertFalse(activity.contains("if (state.bootstrapActivationPending && !state.automationRunning)"))
        assertTrue(viewModel.contains("copy(bootstrapActivationPending = true)"))
        assertTrue(viewModel.contains("requestLocalConsent(AutomationScope.STRICT_SETUP, \"resume-fixed-bootstrap\")"))
        ...
        assertTrue(activation.contains("Settings.GetSettingValue"))
        assertTrue(activation.contains("Addons.SetAddonEnabled"))
        assertTrue(activation.contains("BOOTSTRAP_INSTALLED_DISABLED"))

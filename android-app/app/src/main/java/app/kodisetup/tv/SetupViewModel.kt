...
class SetupViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun beginRealDebrid() = viewModelScope.launch(Dispatchers.IO) {
        ...
        runCatching {
            ...
            user.expiration
        }.onSuccess { expiry -> transition(SetupStep.ACCOUNT_LINK, if (expiry == null) "Real-Debrid linked; no premium expiry was reported" else "Real-Debrid premium active until $expiry", debridExpiry = expiry) }
    ...
    private fun installArtifact(artifact: Artifact?, packageName: String?, target: SetupStep) = viewModelScope.launch(Dispatchers.IO) {
        ...
        runCatching {
            ...
            .onFailure { update(busy = false, error = it.message ?: "Install failed", message = "Package was not installed") }
    }

    private fun monitorInstall(packageName: String, target: SetupStep) = viewModelScope.launch(Dispatchers.IO) {
